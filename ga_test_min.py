...
def getFitnessValue(func, chromosomesdecoded):
    ...
    for i in range(population):  
        ...
        fitnessvalues[i,0] = 10000/(objvalues[i, 0] + 1)
    #寻找精英
    ...
    return objvalues, cum_probability, elite_index 
...
if __name__ ==  '__main__':
    ...
    # 得到个体适应度值和个体的累积概率  
    objValues, cum_proba,elite_index  = getFitnessValue(objFunc(), decoded)  
    #print('累计概率\n',cum_proba)
...
#        print('变异')
        ...
        # 适应度评价  
        objValues, cum_individual_proba,elite_index = getFitnessValue(objFunc(), final_decoded)  
#        print('适应度')
        # 搜索每次迭代的最优解，以及最优解对应的目标函数的取值  
        optimalValues.append(np.min(list(objValues))) 
        print('best fitness: %d'% optimalValues[-1])
        index = np.where(objValues == min(list(objValues)))  
