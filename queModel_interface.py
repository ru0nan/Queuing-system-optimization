...
#        self.acc_wei = alpha*(self.init_pri) + beta*self.wait_time
        self.acc_wei = Patient.alpha*pi[self.init_pri] + Patient.beta*self.wait_time
...
def output(a=6,b=1):
    file = open('arr_ser.npy','rb')
    arr_seq = np.load(file)
    ser_seq = np.load(file)
    file.close()
    ...
    for i in range(duration):
        ...
        num_p1 = int(np.round(num_total * 0.02))
        num_p2 = int(np.round(num_total * 0.07))
        num_p3 = int(np.round(num_total * 0.14))
        num_p4 = num_total - num_p1 - num_p2 - num_p3
